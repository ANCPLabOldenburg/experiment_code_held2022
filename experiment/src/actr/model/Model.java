...
public class Model {
	...
	public void run(boolean con, int n, boolean practice, int trialNum) {
		...
		while (!stop && (events.hasMoreEvents() || runUntilStop)) {
			...
			time = event.getTime();
			taskUpdated = false;
			if (event.getDescription().equals("update")) {
				event.action();
			}
		}
	}
	...
	void setParameter(String parameter, String value, Tokenizer t) {
		...
		else if (parameter.equals(":trout"))
			traceOut = (!value.equals("nil")); // mlh
		else if (parameter.equals(":qs"))
			queuedStuffing = Double.valueOf(value);
	...
	// mlh
	public static void print(List<String> output, boolean construction, int nBack, boolean practice, int trialNum) {

		String cd = new File("").getAbsolutePath() + "/data/";
		try {
			if (!practice) {
				FileWriter writer;
				if (trialNum < 10) {
					String filename = cd + "data(1)_" + nBack + "-back_" + "_construction=" + construction
							+ " _participant" + ServerMain.participantNumber + ".txt";
					writer = new FileWriter(filename);
				} else {
					String filename = cd + "data(2)_" + nBack + "-back_" + "_construction=" + construction
							+ "_participant" + ServerMain.participantNumber + ".txt";
					writer = new FileWriter(filename);
				}
