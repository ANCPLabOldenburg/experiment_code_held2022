...
public class Simulation {
	...
	Simulation(Model model, boolean con) {
		this.model = model;
		driver = new Driver(model, "Driver", 25, 1.0f, 1.0f);
		scenario = new Scenario();
		env = new Env(driver, scenario, con);
	...
	Sample recordSample(Env env) {
		...
		s.visAttention = env.construction.construction ? "construction" : "highway";

		// @MdM added signPassed, signVisible, indicator
		s.signPassed = env.speedsign.signPassed;
		s.signVisible = env.speedsign.visible;

		// Nilma added:
		s.indicator = env.controls.getIndicator();
		return s;
	}
